...
SYSTEM_WINDOW_TITLES: FrozenSet[str] = frozenset({
...
SYSTEM_WINDOW_CLASSES: FrozenSet[str] = frozenset({
...
MEDIA_PLAYER_PROCESSES: FrozenSet[str] = frozenset({
    'mpv', 'vlc', 'mpc', 'potplayer', 'gmp', 'wmplayer', 'kodi', 'jellyfin', 'plex'
})

MEDIA_PLAYER_CLASSES: FrozenSet[str] = frozenset({
...
def is_window_visible(hwnd: int) -> bool:
    ...
    try:
        ...
        _logger.warning("IsWindowVisible failed for hwnd=%s: %s", hwnd, e, exc_info=True)
        return False
...
def get_window_rect(hwnd: int) -> Optional[Tuple[int, int, int, int]]:
    ...
    try:
        ...
        _logger.warning("GetWindowRect failed for hwnd=%s: %s", hwnd, e, exc_info=True)
    return None
...
def get_window_style(hwnd: int) -> int:
    ...
    try:
        ...
        _logger.warning("GetWindowLongW failed for hwnd=%s: %s", hwnd, e, exc_info=True)
        return 0
...
def get_window_title(hwnd: int) -> str:
    ...
    try:
        ...
        _logger.warning("GetWindowTextW failed for hwnd=%s: %s", hwnd, e, exc_info=True)
    return ""
...
def get_window_class(hwnd: int) -> str:
    ...
    try:
        ...
        _logger.warning("GetClassNameW failed for hwnd=%s: %s", hwnd, e, exc_info=True)
    return ""
...
def get_window_process_info(hwnd: int) -> Tuple[int, str]:
    ...
    try:
        ...
        _logger.warning("GetProcessImageFileNameW failed for hwnd=%s: %s", hwnd, e, exc_info=True)
    return (0, "")
...
def is_system_window(hwnd: int, our_pid: int) -> bool:
    ...
    if not process_id or process_id == our_pid:
        return True
        
    if any(proc in exe_name.lower() for proc in SYSTEM_PROCESSES):
        return True
...
def is_media_player(hwnd: int) -> bool:
    ...
    if any(player in exe_name.lower() for player in MEDIA_PLAYER_PROCESSES):
        return True
        
    if any(player.lower() in class_name.lower() for player in MEDIA_PLAYER_CLASSES):
        return True
        
    # Check if title contains video file extensions
...
def is_valid_window(hwnd: int, our_pid: Optional[int] = None, check_visible: bool = True) -> bool:
    ...
    # Use current process ID if not provided
    if our_pid is None:
        our_pid = os.getpid()
    
    # Check if window is visible if required
    ...
    if our_pid:
        process_id, _ = get_window_process_info(hwnd)
