...
class SubSettingsDialog(QDialog):
    ...
    def _update_close_button_style(self):
        """Update the close button style based on theme."""
        if not hasattr(self, 'close_label') or not self.close_label:
            return
            
        is_light = getattr(self, 'theme', 'dark').lower() == 'light'
        self.normal_color = "#000000" if is_light else "#ffffff"
        self.hover_color = "#ffffff" if is_light else "#000000"
        
        # Apply the normal color
        self.close_label.setStyleSheet(f"""
    ...
    def __init__(self, parent=None, app_instance=None, modal=True, flags=None):
        ...
        self.resize(297, 420)  # Reduced width by 10% (from 330px)
        self.move(
        ...
        # Update close button style based on theme
        self._update_close_button_style()
    ...
    def setup_ui(self):
        ...
        # Create a custom close label with manual event handling
        self.close_label = QLabel("✕", self)
        self.close_label.setFixedSize(36, 36)
        self.close_label.setCursor(Qt.ArrowCursor)
        self.close_label.setAlignment(Qt.AlignCenter)
        
        # Set a clean, large font for the X
        font = QFont("Arial", 18, QFont.Bold)
        self.close_label.setFont(font)
        
        # Enable mouse tracking and install event filter
        self.close_label.setAttribute(Qt.WA_Hover, True)
        ...
        self.close_label.mousePressEvent = lambda e: self.close()
        
        # Add title and close button to title layout
        title_layout.addWidget(self.title_label, 1)
        title_layout.addWidget(self.close_label, 0, Qt.AlignRight | Qt.AlignVCenter)
        
        # Connect hover effect
        def enter_event(e):
            self.close_label.setStyleSheet(f"color: {self.hover_color}; font-size: 20px; font-weight: bold;")
            
        def leave_event(e):
            self.close_label.setStyleSheet(f"color: {self.normal_color}; font-size: 20px; font-weight: bold;")
            
        self.close_label.enterEvent = enter_event
        self.close_label.leaveEvent = leave_event
        title_layout.setSpacing(0)
        
        # Add title bar to content layout
        content_layout.addWidget(self.title_bar)
        
        # Apply close button style with a small delay to ensure it overrides theme
        QTimer.singleShot(20, self._update_close_button_style)
        
        # Also update on theme changes
        if hasattr(self, 'theme_combo') and self.theme_combo:
            self.theme_combo.currentTextChanged.connect(
                lambda: QTimer.singleShot(20, self._update_close_button_style)
            )
        ...
        # Stub for future Opacity Hotkeys feature
        self.opacity_hotkeys_checkbox = CircleCheckBox("This Button Is Too Shy To Click Right Now")
        self.opacity_hotkeys_checkbox.setFont(QFont("Segoe UI", 10))
        self.opacity_hotkeys_checkbox.setEnabled(False)  # Disable until implemented
        self.opacity_hotkeys_checkbox.theme = self.theme.lower() if hasattr(self, 'theme') else 'dark'
        content_layout.addWidget(self.opacity_hotkeys_checkbox, alignment=Qt.AlignTop)
        
    def _on_opacity_hotkeys_changed(self, state):
        # Placeholder for future opacity hotkeys functionality
        logger.debug("Opacity hotkeys changed to:", state)
        pass
    ...
    def paintEvent(self, event):
        ...
        if is_light:
            ...
            border_color = QColor(0, 0, 0, 200)  # Black border with fixed opacity
        else:
            bg_color = QColor(30, 30, 30, 200)  # Dark, ~80% opacity
            border_color = QColor(255, 255, 255, 200)  # White border with fixed opacity
    ...
    def save_settings(self):
        ...
        # Keep the setting for backward compatibility but set to False
    ...
    def load_settings(self):
        ...
        try:
            ...
            # Always set aggressive passthrough to False as the feature is removed
            ...
            if hasattr(self, 'hotkey_checkbox') and hasattr(self, 'hotkey_edit'):
                ...
                hotkey_sequence = settings.value("hotkey_sequence", "`", type=str)
            ...
            # No longer need to unblock signals for removed checkbox
            if hasattr(self, 'click_through_checkbox') and self.click_through_checkbox:
    ...
    def _connect_signals(self):
        ...
        if hasattr(self, 'key_passthrough_checkbox'):
            self.key_passthrough_checkbox.stateChanged.connect(self._on_key_passthrough_changed)
        # Connect opacity hotkeys checkbox if it exists
        if hasattr(self, 'opacity_hotkeys_checkbox'):
    ...
    def _on_aggressive_passthrough_changed(self, state):
        ...
        # The aggressive passthrough feature has been removed
        pass
