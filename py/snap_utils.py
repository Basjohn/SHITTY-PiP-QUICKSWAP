...
from PySide6.QtGui import QMouseEvent, QGuiApplication, QScreen
...
# Cache for screen information
_screen_cache = None
_cache_valid = False
# Cache for physical monitor information
_physical_monitor_cache = {}
_physical_monitor_cache_time = 0
_physical_monitor_cache_ttl = 5.0  # Cache TTL in seconds
# Cache for DPI information
_dpi_cache = {}
_dpi_cache_time = 0
_dpi_cache_ttl = 5.0  # Cache TTL in seconds
...
def _refresh_screen_cache():
    """Refresh the screen cache with the latest monitor information.
    
    This function populates the screen cache with monitor geometries, workspaces,
    and other screen-related information. It's called automatically when needed.
    """
    global _screen_cache, _cache_valid, _physical_monitor_cache, _physical_monitor_cache_time
    ...
    try:
        ...
        # Invalidate physical monitor cache if it's too old
        if (current_time - _physical_monitor_cache_time) > _physical_monitor_cache_ttl:
            _physical_monitor_cache.clear()
            _physical_monitor_cache_time = current_time
        
        for screen in screens:
            screen_name = screen.name()
            geometry = screen.geometry()
            
            # Get physical monitor info from cache or compute it
            if screen_name not in _physical_monitor_cache:
                _physical_monitor_cache[screen_name] = get_physical_monitor_info(screen)
            ...
            monitors.append({
                ...
                'device': screen_name,
                'physical_info': _physical_monitor_cache[screen_name],
                'dpi_scale': get_screen_scale_factor(screen)
            })
        ...
        logger.error(f"Error refreshing screen cache: {e}", exc_info=True)
        _screen_cache = {'monitors': [], 'virtual_screen': QRect()}

def get_screen_info():
    """Get the cached screen information, refreshing if necessary.
    
    Returns:
        dict: Cached screen information including monitors and virtual screen bounds
    """
    current_time = time.time()
    
    # Invalidate cache if it's too old or invalid
    if (not _cache_valid or 
        not _screen_cache or 
        (current_time - _screen_cache.get('timestamp', 0)) > _physical_monitor_cache_ttl):
...
def get_cursor_for_edge(edge: Optional[str]) -> Qt.CursorShape:
    """Return the appropriate cursor shape for a given resize edge.
    
    Args:
        edge: The edge being resized, as returned by get_resize_edge_for_pos
        
    Returns:
        Qt cursor shape appropriate for the resize edge
    """
    if edge is None:
        return Qt.ArrowCursor
        
    # Map edge positions to cursor shapes
    cursor_map = {
...
def handle_overlay_mouse_move(event: QMouseEvent, window, state, snap_distance=30):
    try:
        ...
        if state.get('is_resizing'):
            ...
            # Apply snapping and constrain to desktop
            new_pos, _ = apply_snap(new_pos, window.size(), snap_distance)
            new_pos = ensure_within_available_desktop(new_pos, window.size())
...
def invalidate_cache():
    """Invalidate all caches to force a refresh on the next access."""
    global _cache_valid, _physical_monitor_cache_time, _dpi_cache_time
    _cache_valid = False
    # Set cache times to 0 to force refresh
    _physical_monitor_cache_time = 0
    _dpi_cache_time = 0
...
def get_physical_monitor_info(screen: QScreen) -> Dict[str, Any]:
    ...
    for the specified screen, accounting for Windows DPI scaling. Results are cached
    ...
    """
    global _physical_monitor_cache, _physical_monitor_cache_time
    
    # Check cache first
    screen_name = screen.name()
    current_time = time.time()
    
    # Invalidate entire cache if it's too old
    if (current_time - _physical_monitor_cache_time) > _physical_monitor_cache_ttl:
        _physical_monitor_cache.clear()
        _physical_monitor_cache_time = current_time
    
    # Return cached value if available
    if screen_name in _physical_monitor_cache:
        return _physical_monitor_cache[screen_name]
    ...
    # Cache the result
    _physical_monitor_cache[screen_name] = result
...
def get_screen_scale_factor(screen: QScreen) -> float:
    ...
    This function caches the results to improve performance, as DPI information
    doesn't change often during normal operation.
    ...
    """
    global _dpi_cache, _dpi_cache_time
    ...
    # Invalidate entire cache if it's too old
    if (current_time - _dpi_cache_time) > _dpi_cache_ttl:
        _dpi_cache.clear()
        _dpi_cache_time = current_time
    elif screen_name in _dpi_cache:
        return _dpi_cache[screen_name]
