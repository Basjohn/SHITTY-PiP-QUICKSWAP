...
def get_all_monitors() -> List[Dict[str, Any]]:
    try:
        ...
        if not qt_screens:
            ...
            return []
        logger.info(f"Found {len(qt_screens)} Qt screens")
        monitors = []
        for i, screen in enumerate(qt_screens):
            try:
                ...
                logger.debug(f"Added monitor {i}: {monitor_info.get('device_name', 'Unknown')} "
        ...
        for i, mon in enumerate(monitors):
            logger.info(f"Monitor {i}: {mon.get('device_name', 'Unknown')} "
                      f"{mon['rect'].width()}x{mon['rect'].height()}+{mon['position'].x()}+{mon['position'].y()} "
