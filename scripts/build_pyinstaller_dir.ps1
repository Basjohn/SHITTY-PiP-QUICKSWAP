...
$DistDir     = Join-Path $ReleaseRoot 'dist_py_dir'
$SpecDir     = Join-Path $WorkRoot 'spec'
$BuildDir    = Join-Path $WorkRoot 'build'
$LogPath     = Join-Path $ReleaseRoot 'build_pyinstaller_dir.log'
...
# Self-contained venv (works without global deps)
...
$sw.Stop()
Write-Info ("PyInstaller (onedir) finished in {0:N1}s with exit code {1}" -f $sw.Elapsed.TotalSeconds, $proc.ExitCode)

if ($proc.ExitCode -ne 0) { Write-Err "PyInstaller onedir build failed."; if ($TranscriptStarted){ try { Stop-Transcript | Out-Null } catch {} }; exit $proc.ExitCode }

# Validate output
$exePath = Join-Path $DistDir 'SPQ' # folder
if (-not (Test-Path $exePath)) { Write-Err "Missing dist directory: $exePath"; if ($TranscriptStarted){ try { Stop-Transcript | Out-Null } catch {} }; exit 1 }

Write-Info "One-directory build created at: $exePath"
