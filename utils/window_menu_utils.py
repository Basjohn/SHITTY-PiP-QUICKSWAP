...
from utils.debug.performance import log_perf, DebugTimer
...
# Rate limiting for on-demand refreshes (seconds)
MIN_REFRESH_INTERVAL = 1.0
_last_forced_refresh_attempt_ts: float = 0.0

def maybe_refresh_window_list_cache(app_instance, min_interval_sec: float = MIN_REFRESH_INTERVAL) -> None:
    """Rate-limited refresh attempt for the window list cache.
    
    Tries to refresh the cache if at least `min_interval_sec` seconds have
    elapsed since the last attempt. Explicitly logs both refreshes and skips.
    """
    import time as _time
    global _last_forced_refresh_attempt_ts
    try:
        now = _time.time()
        if now - _last_forced_refresh_attempt_ts < max(0.0, float(min_interval_sec)):
            if debug_enabled:
                logger.debug("Window list refresh skipped due to rate limit")
            return
        _last_forced_refresh_attempt_ts = now
        logger.info("Attempting rate-limited window list refresh before menu populate")
        refreshed = refresh_window_list_cache(app_instance)
        if refreshed is None:
            logger.warning("Rate-limited refresh returned no data; menu will use last cached list if available")
    except Exception:
        logger.exception("Error during rate-limited window list refresh")
...
def populate_window_switch_menu(menu: QMenu, app_instance, on_window_selected: Callable[[int], None]):
    ...
    try:
        with DebugTimer("Clearing and populating window switch menu"):
            ...
            # Trigger a rate-limited refresh to avoid stale lists on menu open
            maybe_refresh_window_list_cache(app_instance)

            # Get window list (will use cache if recent or use the just-refreshed data)
