...
from utils.win32constants import ExtendedWindowStyles as WS_EX, WindowStyles as WS

logger = get_logger(__name__)

# Reduce noisy exclusion logs unless explicitly enabled
VERBOSE_EXCLUSION_LOGS = False

def _dbg(msg: str) -> None:
    if VERBOSE_EXCLUSION_LOGS:
        logger.debug(msg)

def set_verbose_exclusion_logs(enabled: bool) -> None:
    """Enable or disable verbose exclusion logging for window filtering.

    Args:
        enabled: When True, emits detailed debug logs for exclusions.
    """
    global VERBOSE_EXCLUSION_LOGS
    VERBOSE_EXCLUSION_LOGS = bool(enabled)
    try:
        logger.info(
            f"[WINFILTER] Verbose exclusion logs {'ENABLED' if VERBOSE_EXCLUSION_LOGS else 'DISABLED'}"
        )
    except Exception:
        # Logging should never raise in hot paths
        pass

def get_verbose_exclusion_logs() -> bool:
    """Return current verbose exclusion logging state."""
    return VERBOSE_EXCLUSION_LOGS
...
class WindowFilter:
    ...
    # Deprecated: process-name whitelisting. We rely on blacklist + heuristics instead.
    ...
    def is_main_window(cls, hwnd):
        ...
        try:
            ...
            if owner != 0:
                ...
                if (ex_style & WS_EX.TOOLWINDOW or 
                    not (ex_style & WS_EX.APPWINDOW) and not (style & WS.OVERLAPPEDWINDOW)):
                    return False
                    
            return True
            
        except Exception as e:
            _dbg(f"Error checking if window {hwnd} is main window: {e}")
            return False
    ...
    def is_valid_window(cls, hwnd: int, our_pid: Optional[int] = None, min_width: int = 50, min_height: int = 30, cache: Optional[dict] = None) -> bool:
        ...
        if not hwnd or hwnd == 0:
            _dbg(f"Window {hwnd}: Invalid window handle")
            return False
        try:
            ...
            if info.pid == our_pid:
                _dbg(f"Window {hwnd}: Excluding our own process window")
                return False
            if not info.visible:
                _dbg(f"Window {hwnd}: Excluding invisible window")
                return False
            ...
            if info.process_name and info.process_name in cls.NEVER_FILTER_PROCESSES:
                _dbg(f"Window {hwnd}: NEVER filtering critical process: {info.process_name}")
                return True
            if info.class_name in ['Shell_TrayWnd', 'Shell_SecondaryTrayWnd']:
                _dbg(f"Window {hwnd}: BLOCKED taskbar window")
                return False
            if info.class_name in cls.SYSTEM_WINDOW_CLASSES:
                _dbg(f"Window {hwnd} (Class: {info.class_name}): Excluded by system window class")
                return False
            if info.title:
                title_lower = info.title.lower()
                for sys_win in cls.SYSTEM_WINDOW_TITLES:
                    if sys_win.lower() in title_lower:
                        _dbg(f"Window {hwnd} (Title: {info.title}): Excluded by system window title: {sys_win}")
                        return False
            if not cls.is_main_window(hwnd):
                _dbg(f"Window {hwnd}: Excluded as child/popup window")
                return False
            ...
            if info.rect:
                ...
                if width <= 0 or height <= 0 or width < min_width or height < min_height:
                    _dbg(f"Window {hwnd}: Excluded due to small size: {width}x{height}")
                    return False
            style = win32gui.GetWindowLong(hwnd, win32con.GWL_STYLE)
            ex_style = win32gui.GetWindowLong(hwnd, win32con.GWL_EXSTYLE)
            if ex_style & WS_EX.TOOLWINDOW and not (ex_style & WS_EX.APPWINDOW):
                if info.process_name and info.process_name in cls.NEVER_FILTER_PROCESSES:
                    _dbg(f"Window {hwnd}: Allowing tool window from critical process: {info.process_name}")
                    return True
                _dbg(f"Window {hwnd}: Excluded tool window without app window flag")
                return False
            if style & WS.DISABLED:
                _dbg(f"Window {hwnd}: Excluded disabled window")
                return False
            if 'firefox' in info.class_name.lower() or 'mozilla' in info.class_name.lower():
                if info.class_name in cls.FIREFOX_EXCLUDED_CLASSES:
                    _dbg(f"Window {hwnd}: Excluded Firefox-specific window class: {info.class_name}")
                    return False
            if info.process_name:
                if info.process_name == 'explorer.exe':
                    if info.class_name in cls.EXPLORER_ALLOWED_CLASSES:
                        _dbg(f"Window {hwnd}: Allowed File Explorer window (Class: {info.class_name})")
                        return True
                    elif info.class_name in cls.SYSTEM_WINDOW_CLASSES:
                        _dbg(f"Window {hwnd}: Blocked explorer.exe system window (Class: {info.class_name})")
                        return False
                    elif info.title and len(info.title.strip()) > 0:
                        _dbg(f"Window {hwnd}: Allowed explorer.exe window with title: '{info.title}'")
                        return True
                    else:
                        _dbg(f"Window {hwnd}: Blocked explorer.exe window without title (Class: {info.class_name})")
                        return False
                # Do not allow by generic process-name whitelist; rely on heuristics only
                if info.process_name not in cls.NEVER_FILTER_PROCESSES:
                    for sys_proc in cls.SYSTEM_PROCESSES:
                        if sys_proc.lower() == info.process_name:
                            _dbg(f"Window {hwnd}: Excluded by system process: {sys_proc}")
                            return False
            if not info.title and not info.class_name:
                _dbg(f"Window {hwnd}: Excluded - no title or class name")
                return False
            _dbg(f"Window {hwnd} (Title: '{info.title}', Class: '{info.class_name}', Process: '{info.process_name}'): INCLUDED")
            return True
        except Exception as e:
            _dbg(f"Error checking window validity for hwnd {hwnd}: {e}")
            return False
